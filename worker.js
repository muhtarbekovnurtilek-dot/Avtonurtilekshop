...
const CATALOG = {
  freefire: {
    ...
    items: [
      ...
      { id: "ff_month", name: "⚡ Month", price: 699 },
    ],
  },
  pubg_uc: {
    name: "PUBG Mobile — UC",
...
async function walletRefundSpend(db, userId, amount, orderNumber) {
  const user = await getUser(db, userId);
  user.spent = Math.max(0, Number(user.spent || 0) - Number(amount || 0));
  await saveUser(db, user);
  await addWalletHistory(db, userId, { type: "purchase_refund", amount, orderNumber });
}
...
async function handleCallbackQuery(env, db, cq) {
  ...
  try {
    ...
    if (ns === "confirmorder") {
      ...
      // Pay from wallet. Do not consume idempotency before checking the balance.
      ...
      if (!first) {
        // A duplicate callback raced with this one. Return the reservation if this request got it.
        await walletRelease(db, userId, order.total, `duplicate_confirm:${order.orderNumber}`);
        await answerCallback(env, cq.id);
        return;
      }
      order.status = "paid";
...
async function processDonixOrder(env, db, order, lang, chatId) {
  ...
  if (!result.ok || !result.data) {
    ...
    if (refundOk) {
      await walletRelease(db, order.userId, order.total, `order_failed:${order.orderNumber}`);
      await walletRefundSpend(db, order.userId, order.total, order.orderNumber);
    }
...
async function handleDonixStatusUpdate(env, db, externalId, status) {
  ...
  if (status === "processing") {
    ...
    if (refundOk) {
      await walletRelease(db, order.userId, order.total, `donix_failed:${order.orderNumber}`);
      await walletRefundSpend(db, order.userId, order.total, order.orderNumber);
    }
    ...
    const refundOk = await idempotentOnce(db, "refund", externalId);
    if (refundOk) {
      await walletRelease(db, order.userId, order.total, `donix_refunded:${order.orderNumber}`);
      await walletRefundSpend(db, order.userId, order.total, order.orderNumber);
    }
...
async function buildOrderConfirmPrompt(env, db, lang, userId, stateData, uidData) {
  ...
  const activePromo = state.data.promoCode || await db.get(kvKeyActivePromo(userId));
  if (activePromo) {
    const check = await validatePromo(db, activePromo, userId, price);
    if (check.ok) {
...
async function handleMessage(env, db, msg) {
  ...
  if (state.step === "await_promo_code") {
    ...
    // Check existence/expiry/usage now; minimum order is checked again when a product is selected.
    const promo = await getJSON(db, kvKeyPromo(code));
    if (!promo || !promo.active || (promo.expiresAt && new Date(promo.expiresAt).getTime() < Date.now()) || promo.usesLeft <= 0) {
      await sendMessage(env, chatId, t(lang, "promo_invalid"), backHomeKeyboard(lang));
      return;
    }
    if (promo.onePerUser && await db.get(kvKeyPromoUse(code, userId))) {
      await sendMessage(env, chatId, t(lang, "promo_invalid"), backHomeKeyboard(lang));
      return;
    }
    if (promo.firstPurchaseOnly) {
    ...
    await db.put(kvKeyActivePromo(userId), code);
    await clearState(db, userId);
    await sendMessage(env, chatId, `✅ ${code}`, backHomeKeyboard(lang));
    return;
  }
...
async function routePaymentWebhook(env, db, request) {
  ...
  const paidAmount = Number(payload.amount ?? payload.fields?.amount ?? 0);
  if (!Number.isFinite(paidAmount) || paidAmount !== Number(topup.amount)) {
    console.log("Finik webhook: amount mismatch", JSON.stringify({ expected: topup.amount, received: paidAmount, paymentId }));
